...
namespace TestHttpServer
{
    ...
    class Program
    {
        ...
        static void Process()
        {
            Dictionary<string, string> userinf = new Dictionary<string, string>();//字典存储用户名，用户要存储的数据
            int count = 1;
            while (true)
            {
                ...
                if (urlmsg == "save")//如果是存档操作
                {
                    ...
                    foreach (var key in keys)//在服务端遍历输出请求URL中的参数，第一次key=user第二次key=data，value皆为传入的值。
                    {
                        ...
                        else if(key == "data") {data= httpListenerContext.Request.QueryString[key]; }//如果收到客户端发来的key为data，将其value值赋给本地data变量
                        //Console.WriteLine(key + ":"+ httpListenerContext.Request.QueryString[key]);
                    }
                   
                        if (userinf.ContainsKey(user))//如果字典已存在同名user,直接修改该user的Value
                        {
                        userinf[user] = data;
                        }
                    else
                    {
                        userinf.Add(user, data);//如果之前没有此user信息，userinf字典追加获得的用户信息
                    }
                ...
                else if (urlmsg == "load")//如果是读档操作
                {
                    ...
                    foreach (var username in userinf.Keys)
                    {
                        if (username == user)//如果找到客户端发送来的用户，返回字典中该key的value
                        {
                            backMsg = userinf[username];
                        }
                    }
                    httpListenerContext.Response.StatusCode = 200;//返回请求已成功状态码
